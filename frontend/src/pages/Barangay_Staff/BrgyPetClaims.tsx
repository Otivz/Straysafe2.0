...
const BrgyPetClaims = () => {
    ...
    const [claims, setClaims] = useState<any[]>(() => getCachedData<any[]>('brgy_pet_claims') || []);
    ...
    const fetchBackendClaims = async () => {
        try {
            ...
            if (!getCachedData('brgy_pet_claims')) {
                setClaims([]);
            }
        }
    };
                                ...
                                <div className="lg:col-span-7 space-y-5">
                                                ...
                                                <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 min-h-[80px] space-y-3">
                                                    {selectedClaim.distinctive_markings && (
                                                        <div className="pb-2 border-b border-gray-250">
                                                            <span className="text-[8px] font-black uppercase text-[#F97316] tracking-wider block mb-0.5">Distinctive Markings</span>
                                        ...
                                        <div className="grid grid-cols-3 gap-3 mt-3">
                                            ...
                                            <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                                ...
                                                <div className="flex items-center gap-1 mt-0.5">
                                                    <span className="text-[9px] font-black text-gray-400 uppercase">Jurisdiction Match:</span>
                                    ...
                                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Please provide additional owner-pet photos or documents..."
                                    ...
                                    {selectedClaim.status !== 'Potential Owner Match' && selectedClaim.status !== 'Possible Match Found' && (
                                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                                            ...
                                            {selectedClaim.status === 'Handover Complete' || selectedClaim.status === 'Pet Received' ? (
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row gap-2">
                                                        ...
                                                        <Button
                                                            disabled={isSubmitting}
                                                            onClick={() => navigate(`/brgy/messages?reportId=${selectedClaim.report_id}&openMatch=true`)}
                                            ...
                                            <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap items-center justify-between gap-2 text-[9px] text-gray-400 font-medium">
                                                <span>Reviewed By: <strong className="text-gray-600">{staffUser?.name || 'Barangay Officer'}</strong> (Barangay Staff)</span>
                                                <span>Review Date: {new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
