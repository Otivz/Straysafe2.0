...
const SubdPetClaims = () => {
    ...
    // Auto open claim review when navigated from dashboard with claim_id param or location state
    useEffect(() => {
        const targetId = searchParams.get('claim_id') || location.state?.claimId;
        if (targetId && claims.length > 0) {
            const found = claims.find((c: any) => String(c.claim_id) === String(targetId));
            if (found) {
                openReview(found);
            }
        }
    }, [claims, searchParams, location.state]);
    ...
    const filteredClaims = claims.filter(c => {
        ...
        const matchesQ = c.pet?.pet_name?.toLowerCase().includes(q) || 
                         c.pet?.owner?.name?.toLowerCase().includes(q) || 
                         c.pet?.owner?.email?.toLowerCase().includes(q) ||
        ...
        if (dateFilter !== 'All Time' && c.created_at) {
            try {
    ...
    return (
        <div className="min-h-screen w-full flex bg-[#F8F9FA] font-sans text-gray-800">
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden">
                <SubdNavbar onMenuToggle={() => setMobileMenuOpen(true)} leftContent={
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 pb-36 md:pb-6 flex flex-col gap-5 custom-scrollbar bg-[#FAFAF9]">
