...
const SubdNavbar = ({ leftContent, notifications: propNotifications, onNotificationClick, onMenuToggle }: SubdNavbarProps) => {
    ...
    const user = {
        ...
        subdivision_name: parsedUser?.subdivision_name,
    ...
    const openNotificationDetail = async (notif: NotificationItem) => {
        if (!notif.is_read) {
            try {
    ...
    return (
        <header className="h-16 sm:h-20 bg-white/95 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-3 sm:px-6 lg:px-8 sticky top-0 z-[2000] w-full shadow-sm">
            ...
            <div className="flex items-center gap-1 sm:gap-2.5 ml-auto shrink-0">
                ...
                <div className="relative md:hidden" ref={messagesRef}>
                    <button
                        type="button"
                        onClick={() => {
                ...
                <div className="relative" ref={notifRef}>
                    ...
                    {isNotifOpen && (() => {
                        const notifPanel = (
                            <div className={
                                isMobile
                                    ? "fixed inset-0 w-full h-[100dvh] bg-white z-[999999] flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-200"
                                    : "absolute right-0 sm:-right-8 md:right-0 mt-3 w-[25rem] bg-white rounded-2xl shadow-2xl border border-gray-100/90 overflow-hidden z-50 flex flex-col animate-in fade-in zoom-in-95 duration-200"
                            }>
                                ...
                                {/* Filter Tabs */}
                                <div className="px-4 py-2.5 border-b border-gray-100 bg-gray-50/70 flex items-center gap-2 shrink-0">
                                    <button
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center justify-between gap-2">
                                                            <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                                                                ...
                                                                <h4 className={`text-xs tracking-tight truncate ${isTransferAccepted
                                                        ...
                                                        <p className={`text-xs mt-1 leading-relaxed line-clamp-2 ${isTransferAccepted
                                                        ...
                                                        <div className="flex items-center justify-between mt-2 pt-1 border-t border-gray-100/50">
                                                            <button
                                                                ...
                                                                className={`text-[10px] font-bold hover:underline flex items-center gap-1 cursor-pointer ${isTransferAccepted
                        ...
                        return isMobile ? createPortal(notifPanel, document.body) : notifPanel;
                    })()}
                </div>
            ...
            {activeChatThread && (
                <ReportChatDrawer
                    ...
                    report={{
                        report_id: activeChatThread.report_id || 0,
            ...
            {selectedNotifForDetail && (() => {
                ...
                const isTransferRejected = typeStr.includes('rejected') || titleStr.includes('declined') || titleStr.includes('rejected');
                const isTransferRequest = typeStr.includes('transfer_request') || (typeStr.includes('transfer') && titleStr.includes('request'));
                const isHazard = typeStr.includes('alert') || titleStr.includes('alert') || titleStr.includes('hazard') || titleStr.includes('warning');
                const isRescueOrStatus = typeStr.includes('status') || titleStr.includes('update') || titleStr.includes('rescue');

                let bannerGrad = 'from-[#0F172A] via-[#1E293B] to-[#334155]';
                let iconEmoji = '🔔';
                let typeLabel = 'Notification';

                if (isTransferAccepted) {
                    bannerGrad = 'from-emerald-700 via-emerald-800 to-slate-900';
                    iconEmoji = '✅';
                    typeLabel = 'Transfer Accepted';
                } else if (isTransferRejected) {
                    bannerGrad = 'from-rose-700 via-rose-800 to-slate-900';
                    iconEmoji = '❌';
                    typeLabel = 'Transfer Declined';
                } else if (isTransferRequest) {
                    bannerGrad = 'from-purple-700 via-purple-800 to-slate-900';
                    iconEmoji = '🔄';
                    typeLabel = 'Transfer Request';
                } else if (isHazard) {
                    bannerGrad = 'from-amber-600 via-rose-600 to-slate-900';
                    iconEmoji = '⚠️';
                    typeLabel = 'Community Alert';
                } else if (isRescueOrStatus) {
                    bannerGrad = 'from-[#0F172A] via-[#1E293B] to-[#334155]';
                    iconEmoji = '🛡️';
                    typeLabel = 'Case Status Update';
                }
