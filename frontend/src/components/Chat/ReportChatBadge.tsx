...
export default function ReportChatBadge({
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className={`${isSmall ? 'px-1.5 py-0.5' : 'px-2 py-1'} bg-orange-50 dark:bg-orange-950/50 hover:bg-orange-100 dark:hover:bg-orange-900/60 text-[#F97316] dark:text-orange-400 border border-orange-200/80 dark:border-orange-500/40 rounded-lg hover:scale-105 active:scale-95 transition-all shadow-2xs cursor-pointer flex items-center gap-1 shrink-0`}
        ...
        >
            ...
            {unreadCount > 0 && (
                <span className={`${isSmall ? 'text-[9px]' : 'text-[10px]'} font-black text-[#F97316] dark:text-orange-400 leading-none`}>
                    ({unreadCount})
                </span>
            )}
        </button>
